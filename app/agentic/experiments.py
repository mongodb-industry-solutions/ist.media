...
class Experiments:
    ...
    def create_from_plan(self, experiment_class: str, plan: Dict[str, Any]) -> List[ObjectId]:
        ...
        for ex in plan.get("experiments", []) or []:
            doc = {
            ...
            self.store.experiments.update_one({"_id": exp_doc["_id"]}, {"$set": {"arm_ids": arm_ids, "ts_updated": utcnow()}})
            created.append(exp_doc["_id"])
        return created
    def pick_experiment(self, experiment_class: str):
        return self.store.experiments.find_one({"experiment_class": experiment_class, "status": "active"}, sort=[("ts_updated", -1)])
